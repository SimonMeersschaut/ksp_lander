...
if __name__ == '__main__':
  # (thrust, mass, final_height, max_throttle, time_fraction=1)
  simulation = simulate(vessel.max_thrust, vessel.mass, flight.surface_altitude, max_throttle=0.9, time_fraction=1)
  # simulation = simulate(conn.space_center.ut, vessel.max_thrust, vessel.mass, vessel.orbit, max_speeds, max_throttle=0.9, time_fraction=0.1)
  input('simulation V')
  while True:
    
    target = get_target()
    
    F = (get_speed()-target['velocity'])*vessel.mass + vessel.mass*9.81
